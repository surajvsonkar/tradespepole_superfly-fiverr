...
import { ArrowLeft, User, Mail, MapPin, Calendar, Edit2, Save, X, Pause, Play, Trash2, Zap, AlertTriangle, UserCheck, Star, MessageCircle, Heart, Users, Eye, CheckCircle } from 'lucide-react';
...
const HomeownerProfile = () => {
  ...
  const [selectedProjectForDetails, setSelectedProjectForDetails] = useState<JobLead | null>(null);
  const [selectedTradesperson, setSelectedTradesperson] = useState<AppUser | null>(null);
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-6">
          ...
          <p className="text-gray-600 mt-2">Manage your account and view your projects</p>
        </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center">
                  ...
                  {isEditing ? (
                    ...
                    <span className="text-gray-900">{state.currentUser.name}</span>
                  )}
                </div>
                ...
                <div className="flex items-center">
                  ...
                  {isEditing ? (
                    ...
                    <span className="text-gray-900">{state.currentUser.email}</span>
                  )}
                </div>
                ...
                <div className="flex items-center">
                  ...
                  {isEditing ? (
                    ...
                    <span className="text-gray-900">{state.currentUser.location || 'Not specified'}</span>
                  )}
                </div>
                      ...
                      <div key={project.id} className="border border-gray-200 rounded-lg p-4 relative">
                                    ...
                                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-3 sm:space-y-0">
                                      <div className="flex items-center space-x-3 min-w-0 flex-1">
                                        ...
                                        <div className="min-w-0 flex-1">
                                          <p className="font-medium text-gray-900 truncate">{tradesperson.name}</p>
                        ...
                        {hiredTradesperson && (
                          <div className="mt-4 pt-4 border-t border-gray-100">
                            <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                              <h4 className="font-semibold text-green-800 mb-2">✓ Professional Hired</h4>
                              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-3 sm:space-y-0">
                                <div className="min-w-0 flex-1">
                                  <p className="font-medium text-green-900">{hiredTradesperson.name}</p>
                                  <p className="text-sm text-green-700">{hiredTradesperson.trades?.join(', ')}</p>
                                </div>
                                <div className="flex space-x-2">
                                  <button
                                    onClick={() => {
                        ...
                        {project.interests.length > 0 && (
                          <div className="mt-3 pt-3 border-t border-gray-100">
                            <p className="text-sm font-medium text-gray-700 mb-2">Recent Interests:</p>
                            {project.interests.slice(0, 2).map((interest) => (
                              <div key={interest.id} className="bg-purple-50 rounded-lg p-4 mb-2">
                                <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between space-y-3 sm:space-y-0">
                                  <div className="min-w-0 flex-1">
                                    <p className="font-medium text-purple-900">{interest.tradespersonName}</p>
                                    <p className="text-sm text-purple-700 break-words">{interest.message.substring(0, 100)}...</p>
                                    <p className="text-xs text-purple-600">{interest.date}</p>
                                  </div>
                                  <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2 flex-shrink-0">
                                    ...
                                    {interest.status === 'accepted' && (
                                      <span className="text-green-600 text-sm font-medium bg-green-100 px-3 py-2 rounded-lg">✓ Accepted</span>
                                    )}
                                  </div>
                                </div>
                              </div>
                        ...
                        >
                            ...
                            View All Responses ({project.purchasedBy.length + project.interests.length})
                        </button>
                      </div>
                    );
                  })}
        ...
        <div className="mt-8 bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          <div className="mt-4 text-sm text-gray-500">
            <p><strong>Park Account:</strong> Temporarily hide your profile while keeping all data safe</p>
            <p><strong>Boost Profile:</strong> Increase visibility to get more quotes from tradespeople</p>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-xl p-6 max-w-md w-full mx-4">
              ...
              <p className="text-gray-600 mb-6">
                Parking your account will temporarily hide your profile from tradespeople. 
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-xl p-6 max-w-5xl w-full max-h-[90vh] overflow-y-auto">
              <div className="flex justify-between items-center mb-6">
                <div>
                    <h3 className="text-2xl font-bold text-gray-900">{selectedProjectForDetails.title}</h3>
                    <p className="text-gray-600">Manage responses and hire professionals</p>
                </div>
                <button onClick={() => setSelectedProjectForDetails(null)} className="text-gray-500 hover:text-gray-700">
                  <X className="w-6 h-6" />
                </button>
              </div>
              ...
              <div className="mb-8">
                <h4 className="text-lg font-semibold text-gray-800 mb-4 flex items-center">
                   ...
                   Professionals Available to Hire ({selectedProjectForDetails.purchasedByDetails?.length || 0})
                </h4>
                {!selectedProjectForDetails.purchasedByDetails || selectedProjectForDetails.purchasedByDetails.length === 0 ? (
                    <div className="bg-gray-50 rounded-lg p-8 text-center border border-gray-200 border-dashed">
                        <p className="text-gray-500">No professionals have purchased this lead yet.</p>
                    </div>
                ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {selectedProjectForDetails.purchasedByDetails.map(tradesperson => (
                            <div key={tradesperson.id} className="bg-white border border-gray-200 rounded-xl p-5 hover:shadow-md transition-shadow flex flex-col h-full">
                                <div className="flex items-center mb-4">
                                    <div className="w-12 h-12 bg-blue-100 text-blue-600 rounded-full flex items-center justify-center font-bold text-lg mr-3">
                                        {tradesperson.name.charAt(0)}
                                    </div>
                                    <div>
                                        <h5 className="font-semibold text-gray-900">{tradesperson.name}</h5>
                                        <div className="flex items-center text-sm text-gray-500">
                                ...
                                <p className="text-sm text-gray-600 mb-4 flex-grow">
                                    {tradesperson.trades?.join(', ') || 'No specialties listed'}
                                </p>
                                <div className="flex space-x-2 mt-auto">
                                    ...
                                    {!selectedProjectForDetails.hiredTradesperson && selectedProjectForDetails.isActive && (
                                        <button
                                            onClick={() => handleHireTradesperson(selectedProjectForDetails.id, tradesperson.id)}
              ...
              <div>
                <h4 className="text-lg font-semibold text-gray-800 mb-4 flex items-center">
                   ...
                   Expressed Interests ({selectedProjectForDetails.interests.length})
                </h4>
                {selectedProjectForDetails.interests.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {selectedProjectForDetails.interests.map(interest => (
                            <div key={interest.id} className="bg-purple-50 border border-purple-100 rounded-xl p-5 hover:shadow-md transition-shadow flex flex-col h-full">
                                <div className="flex justify-between items-start mb-3">
                                    <h5 className="font-semibold text-purple-900">{interest.tradespersonName}</h5>
                                    <span className="text-xs text-purple-600 bg-purple-100 px-2 py-1 rounded-full">{interest.date}</span>
                                </div>
                                <p className="text-sm text-purple-800 mb-4 flex-grow italic">"{interest.message}"</p>
                                <div className="flex space-x-2 mt-auto">
                                    {interest.status === 'pending' && selectedProjectForDetails.isActive ? (
                                        <>
                                            <button
                                                onClick={() => {
                                                    if (interest.tradespersonDetails) {
                                                        setSelectedTradesperson(interest.tradespersonDetails);
                                                    }
                                                }}
                                                className="flex-1 bg-gray-100 text-gray-700 px-3 py-2 rounded-lg hover:bg-gray-200 transition-colors flex items-center justify-center text-sm font-medium"
                                            >
                                            ...
                                            <button
                                                onClick={() => handleAcceptInterest(selectedProjectForDetails.id, interest.id)}
                                        ...
                                        <>
                                            ...
                                            {!selectedProjectForDetails.hiredTradesperson && selectedProjectForDetails.isActive && (
                                                <button
                                                    onClick={() => handleHireTradesperson(selectedProjectForDetails.id, interest.tradespersonId)}
                                                    className="flex-1 bg-green-600 text-white px-3 py-2 rounded-lg hover:bg-green-700 transition-colors flex items-center justify-center text-sm font-medium"
                                                >
                                                    <UserCheck className="w-4 h-4 mr-1" />
                                                    Hire
                ...
                <div className="bg-white rounded-xl p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
                    <div className="flex justify-between items-start mb-6">
                        <div className="flex items-center">
                            ...
                            <div>
                                <h3 className="text-2xl font-bold text-gray-900">{selectedTradesperson.name}</h3>
                                ...
                                <div className="flex items-center mt-1">
                                    ...
                                    <span className="font-medium text-gray-900 mr-1">
                                        {selectedTradesperson.rating ? Number(selectedTradesperson.rating).toFixed(1) : '0.0'}
                                    </span>
                    ...
                    <div className="space-y-6">
                        <div>
                            ...
                            <p className="text-gray-600 leading-relaxed">
                                {selectedTradesperson.companyDescription || "No description available."}
                            </p>
                        </div>
