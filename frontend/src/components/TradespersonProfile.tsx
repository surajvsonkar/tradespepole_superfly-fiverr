...
const TradespersonProfile = () => {
  ...
  const [companyDescription, setCompanyDescription] = useState(
    state.currentUser?.companyDescription || '20 plus years in home renovations, we supply and fit materials at below only jobs domestic and commercial.'
  );
  ...
  const [selectedTrades, setSelectedTrades] = useState<string[]>(state.currentUser?.trades || []);
  const [selectedServices, setSelectedServices] = useState<string[]>(state.currentUser?.trades || []);
  ...
  const jobsWon = state.jobLeads.filter(lead => 
    lead.hiredTradesperson === state.currentUser?.id
  );
        ...
        <div className="max-w-3xl mx-auto p-6 text-center">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
            ...
            <p className="text-gray-600 mb-6">
              This profile is exclusively for tradespeople. 
  ...
  const sidebarItems = [
    {
      id: 'home-counties',
      label: state.currentUser.businessName || state.currentUser.name.toUpperCase(),
      icon: Building,
      type: 'header'
    },
    {
  ...
  const handleAddPortfolioItem = () => {
    if (!portfolioData.title || !portfolioData.description || !portfolioData.image) {
  ...
  const handleSaveTrades = (type: 'leads' | 'services') => {
  ...
  const purchasedLeads = state.jobLeads.filter(lead => {
    ...
    const hasAcceptedInterest = lead.interests.some(
      interest => interest.tradespersonId === state.currentUser.id && interest.status === 'accepted'
    );
    
    console.log(`Lead ${lead.id}: purchased=${hasPurchased}, acceptedInterest=${hasAcceptedInterest}`);
  ...
  const acceptedInterestLeads = state.jobLeads.filter(lead => 
    lead.interests.some(interest => 
      interest.tradespersonId === state.currentUser.id && interest.status === 'accepted'
    )
  );

  // Get activity data
  const expressedInterests = state.jobLeads.flatMap(lead => 
    lead.interests.filter(interest => interest.tradespersonId === state.currentUser.id)
  );
  
  const hiredJobs = state.jobLeads.filter(lead => 
    lead.hiredTradesperson === state.currentUser.id
  );
                ...
                <div className="space-y-3">
                  <label className="flex items-start">
                    ...
                    <div>
                      ...
                      <div className="text-sm text-gray-600">Homeowners are more likely to hire professionals who offer guarantees</div>
                    </div>
                  </label>
              ...
              <div className="flex items-center">
                {[1, 2, 3, 4, 5].map((star) => (
                  <Star
                    key={star}
                    className={`w-4 h-4 ${
                      star <= (state.currentUser.rating || 0) ? 'text-yellow-400 fill-current' : 'text-gray-300'
                    }`}
                  />
                ))}
              ...
              {userReviews.length === 0 ? (
                ...
                <div className="space-y-4">
                  {userReviews.map((review) => {
                    const reviewer = state.users.find(u => u.id === review.homeownerId);
                        ...
                        <div className="flex items-start justify-between mb-4">
                          <div className="flex items-center">
                            ...
                            <div>
                              ...
                              <p className="text-sm text-gray-500">{job?.title || 'Project'}</p>
                            </div>
                          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              {acceptedInterestLeads.map((lead) => {
                const myInterest = lead.interests.find(interest => 
                  interest.tradespersonId === state.currentUser!.id && interest.status === 'accepted'
                );
                
                return (
                  <div key={`interest-${lead.id}`} className="border border-green-200 rounded-lg p-4 bg-green-50">
                    ...
                    <div className="bg-white border border-green-200 rounded-lg p-4 mb-4">
                      <h4 className="font-semibold text-green-800 mb-2">✅ Contact Details Unlocked</h4>
              ...
              {purchasedLeads.map((lead) => {
                const hasAcceptedInterest = lead.interests.some(interest => 
                  interest.tradespersonId === state.currentUser.id && 
                  interest.status === 'accepted'
                );
                const wasPurchased = lead.purchasedBy.includes(state.currentUser.id);
                  ...
                  <div key={lead.id} className={`border rounded-lg p-4 ${
                    ...
                    <div className="flex justify-between items-start mb-3">
                      <div>
                        ...
                        <p className="text-sm text-gray-600 mb-2">{lead.description.substring(0, 100)}...</p>
                        ...
                        return (
                          <div className="bg-green-50 border border-green-200 rounded-lg p-4 mt-4">
                            <h4 className="font-semibold text-green-800 mb-2">
                              Contact Details {hasAcceptedInterest && !hasPurchased && '(Unlocked via Interest)'}
                            </h4>
                    ...
                    <div className="mt-4">
                      <button
                        onClick={() => {
                          console.log('Creating conversation for lead:', lead.id);
                          const homeowner = state.users.find(u => u.id === lead.postedBy);
                          if (!homeowner) {
                            console.error('Homeowner not found for lead:', lead.postedBy);
                            return;
                          }
              ...
              {purchasedLeads.length === 0 && acceptedInterestLeads.length === 0 && (
                <div className="bg-gray-50 rounded-lg p-8 text-center">
                  <FileText className="w-12 h-12 text-gray-400 mx-auto mb-4" />
                  <p className="text-gray-600">No purchased leads yet</p>
                  <p className="text-sm text-gray-500 mt-2">Leads you purchase will appear here with contact details</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="bg-purple-50 border border-purple-200 rounded-lg p-4 text-center">
                <div className="text-2xl font-bold text-purple-600">{expressedInterests.length}</div>
                <div className="text-sm text-purple-800">Expressed Interests</div>
              </div>
              <div className="bg-green-50 border border-green-200 rounded-lg p-4 text-center">
                <div className="text-2xl font-bold text-green-600">{hiredJobs.length}</div>
                <div className="text-sm text-green-800">Jobs Won</div>
              </div>
              <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 text-center">
                <div className="text-2xl font-bold text-gray-600">{dismissedJobs.length}</div>
            ...
            <div className="space-y-4">
              ...
              {/* Hired Jobs */}
              {hiredJobs.map((job) => (
                <div key={`hired-${job.id}`} className="bg-white border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow cursor-pointer">
                  <div className="flex items-start justify-between">
                    <div className="flex items-start space-x-3">
                      <div className="w-3 h-3 bg-green-500 rounded-full mt-2"></div>
                      <div>
                        <h4 className="font-semibold text-gray-900">Hired for: {job.title}</h4>
                        <p className="text-sm text-gray-600">{job.location} • {job.budget}</p>
                        <p className="text-xs text-gray-500">Posted {job.postedDate}</p>
                      </div>
                    </div>
                    <span className="px-2 py-1 bg-green-100 text-green-800 text-xs rounded-full">Hired</span>
                  </div>
                </div>
              ))}
              ...
              {/* Purchased Leads */}
              {purchasedLeads.map((lead) => (
                <div key={`purchased-${lead.id}`} className="bg-white border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow cursor-pointer">
                  <div className="flex items-start justify-between">
                    <div className="flex items-start space-x-3">
                      <div className="w-3 h-3 bg-blue-500 rounded-full mt-2"></div>
                      <div>
                        <h4 className="font-semibold text-gray-900">Purchased: {lead.title}</h4>
                        <p className="text-sm text-gray-600">{lead.location} • {lead.budget}</p>
                        <p className="text-xs text-gray-500">Posted {lead.postedDate}</p>
                      </div>
                    </div>
                    <span className="px-2 py-1 bg-blue-100 text-blue-800 text-xs rounded-full">£{lead.price}</span>
                  </div>
                </div>
              ))}
              ...
              {expressedInterests.map((interest) => {
                const job = state.jobLeads.find(j => j.interests.some(i => i.id === interest.id));
                return (
                  <div key={`interest-${interest.id}`} className="bg-white border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow cursor-pointer">
                    <div className="flex items-start justify-between">
                      <div className="flex items-start space-x-3">
                        <div className="w-3 h-3 bg-purple-500 rounded-full mt-2"></div>
                        <div>
                          <h4 className="font-semibold text-gray-900">Interest: {job?.title}</h4>
                          <p className="text-sm text-gray-600">{job?.location} • {job?.budget}</p>
                          <p className="text-xs text-gray-500">Expressed {interest.date}</p>
                        </div>
                      </div>
              ...
              {dismissedJobs.map((job) => (
                <div key={`dismissed-${job.id}`} className="bg-white border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow cursor-pointer opacity-75">
                  <div className="flex items-start justify-between">
                    <div className="flex items-start space-x-3">
                      <div className="w-3 h-3 bg-gray-400 rounded-full mt-2"></div>
                      <div>
                        <h4 className="font-semibold text-gray-900">Dismissed: {job.title}</h4>
                        <p className="text-sm text-gray-600">{job.location} • {job.budget}</p>
                        <p className="text-xs text-gray-500">Posted {job.postedDate}</p>
                      </div>
                    </div>
                    <span className="px-2 py-1 bg-gray-100 text-gray-800 text-xs rounded-full">Dismissed</span>
                  </div>
                </div>
              ))}

              {/* No Activity */}
              {purchasedLeads.length === 0 && expressedInterests.length === 0 && hiredJobs.length === 0 && dismissedJobs.length === 0 && (
                <div className="bg-gray-50 rounded-lg p-8 text-center">
                  <Users className="w-12 h-12 text-gray-400 mx-auto mb-4" />
                  <p className="text-gray-600">No activity yet</p>
                  <p className="text-sm text-gray-500 mt-2">Your job activity will appear here</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="space-y-6">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center">
                  <Shield className={`w-6 h-6 mr-3 ${
                    state.currentUser.verified ? 'text-green-600' : 'text-gray-400'
                  }`} />
                  <div>
                    <p className="font-medium text-gray-900">
                      {state.currentUser.verified ? 'Verified Professional' : 'Verification Pending'}
                    </p>
            ...
            {state.currentUser.accountStatus === 'parked' && (
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                <div className="flex items-center">
                  ...
                  <span className="text-yellow-800 font-medium">Your account is currently parked</span>
                </div>
                <p className="text-yellow-700 text-sm mt-1">
                  Parked on {new Date(state.currentUser.parkedDate!).toLocaleDateString()}. 
            ...
            {(purchasedLeads.length === 0 && acceptedInterestLeads.length === 0) ? (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <button
                  onClick={() => dispatch({ type: 'SET_VIEW', payload: 'boost' })}
              ...
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                <div className="flex items-center">
                  ...
                  <span className="text-yellow-800 font-medium">Account management restricted</span>
                </div>
                <p className="text-yellow-700 text-sm mt-1">
                  You have active leads or ongoing projects. Complete or resolve these before managing your account.
                </p>
              </div>
            )}
          </div>
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
              ...
              {state.currentUser.workingArea && (
                <div className="mb-4 text-sm text-blue-700">
                  <p><strong>Current area:</strong> {state.currentUser.workingArea.centerLocation}</p>
                  <p><strong>Radius:</strong> {state.currentUser.workingArea.radius} miles</p>
                </div>
              )}
              <button
                ...
                className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition-colors"
              >
                {state.currentUser.workingArea ? 'Update Work Area' : 'Set Work Area'}
              </button>
            </div>
          </div>
        );
      ...
      case 'lead-settings':
        return (
          <div className="space-y-6">
            ...
            <div className="bg-white border border-gray-200 rounded-lg p-6">
              <h3 className="text-lg font-semibold text-gray-900 mb-4">Select Trades for Lead Notifications</h3>
              <p className="text-gray-600 mb-4">Choose which types of jobs you want to receive notifications for:</p>
              
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 mb-6">
                {availableTrades.map((trade) => (
                  <label key={trade} className="flex items-center p-3 border border-gray-200 rounded-lg hover:bg-gray-50 cursor-pointer">
                    <input
                      type="checkbox"
      ...
      case 'services':
        return (
          <div className="space-y-6">
            ...
            <div className="bg-white border border-gray-200 rounded-lg p-6">
              <h3 className="text-lg font-semibold text-gray-900 mb-4">Your Service Offerings</h3>
              <p className="text-gray-600 mb-4">Select all the services you provide to customers:</p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 mb-6">
                {availableTrades.map((trade) => (
                  <label key={trade} className="flex items-center p-3 border border-gray-200 rounded-lg hover:bg-gray-50 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={selectedServices.includes(trade)}
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto flex flex-col lg:flex-row">
        ...
        <div className="w-full lg:w-80 bg-white border-b lg:border-b-0 lg:border-r border-gray-200 lg:min-h-screen">
          <div className="p-4">
            <h1 className="text-xl lg:text-2xl font-bold text-gray-900 mb-4 lg:mb-6">Profile</h1>
            <nav className="space-y-1">
              {sidebarItems.map((item) => {
                if (item.type === 'header') {
                  return (
                    <div key={item.id} className="flex items-center py-3 px-3 bg-gray-50 rounded-lg mb-4 lg:mb-4">
      ...
      {showPortfolioModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl p-6 max-w-md w-full max-h-[90vh] overflow-y-auto">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">Add Portfolio Item</h3>
                ...
                <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center">
                  {portfolioData.image ? (
                    <div>
                      ...
                      />
                      <p className="text-sm text-gray-600">{portfolioData.image.name}</p>
                      <button
                        onClick={() => setPortfolioData({...portfolioData, image: null})}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 max-w-md w-full mx-4">
            <div className="flex items-center mb-4">
              ...
              <h3 className="text-lg font-semibold text-gray-900">Park Your Account</h3>
            </div>
            <p className="text-gray-600 mb-6">
              Parking your account will temporarily hide your profile from homeowners. 
              You can reactivate anytime and go live instantly.
            </p>
