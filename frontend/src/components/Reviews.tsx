...
const Reviews = () => {
  ...
  const displayReviews = state.reviews.map(review => ({
      name: review.homeowner?.name || 'Homeowner',
      location: review.homeowner?.location || 'London',
      rating: review.rating,
      review: review.comment,
      tradesperson: `${review.tradesperson?.name || 'Tradesperson'} - ${review.tradesperson?.trades?.[0] || 'Professional'}`,
      avatar: review.homeowner?.avatar || 'https://images.pexels.com/photos/91227/pexels-photo-91227.jpeg?auto=compress&cs=tinysrgb&w=400'
  }));

  // Fallback to static reviews if no API reviews found
  const reviews = displayReviews.length > 0 ? displayReviews : [
    {
